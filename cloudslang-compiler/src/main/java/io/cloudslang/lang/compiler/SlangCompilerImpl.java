...
public class SlangCompilerImpl implements SlangCompiler {
    ...
    public Set<SystemProperty> loadSystemProperties(SlangSource source) {
        try {
            ...
            throw new RuntimeException(ERROR_LOADING_PROPERTIES_FILE_MESSAGE + source.getFileName() + "\'. Nested exception is: " + ex.getMessage(), ex);
        }
    }
    ...
    private Set<SystemProperty> extractProperties(ParsedSlang parsedSlang) {
        ...
        for (Map<String, Object> propertyAsMap : parsedSystemProperties) {
            ...
            if (SetUtils.containsIgnoreCase(modelledSystemPropertyKeys, propertyKey)) {
                throw new RuntimeException(DUPLICATE_SYSTEM_PROPERTY_KEY_ERROR_MESSAGE_PREFIX + propertyKey + "\'.");
            } else {
                ...
                if (propertyAsObject instanceof Map) {
                    ...
                    if (propertyAsMap.size() == 1) {
                        ...
                        if (propertyKeyAsObject instanceof String) {
                            ...
                            throw new RuntimeException(SYSTEM_PROPERTY_KEY_WRONG_TYPE_ERROR_MESSAGE_PREFIX + propertyKeyAsObject + "(" + propertyKeyAsObject.getClass().getName() + ").");
                        }
                    } else {
                        throw new RuntimeException(SIZE_OF_SYSTEM_PROPERTY_ERROR_MESSAGE_PREFIX + propertyAsMap + "\' size is: " + propertyAsMap.size() + ".");
                    }
                } else {
                    ...
                    throw new RuntimeException(PROPERTY_LIST_ELEMENT_WRONG_TYPE_ERROR_MESSAGE_PREFIX + errorMessageSuffix);
                }
            }
        } else {
            throw new RuntimeException("Under \'" + SlangTextualKeys.SYSTEM_PROPERTY_KEY + "\' key there should be a list. Found: " + propertiesAsObject.getClass().getName() + ".");
        }
        ...
        if (rawValue instanceof Map) {
            ...
            for (String modifierKey : modifiers.keySet()) {
                if (!knownModifierKeys.contains(modifierKey)) {
                    throw new RuntimeException("Artifact {" + key + "} has unrecognized tag {" + modifierKey + "}" + ". Please take a look at the supported features per versions link");
                }
            }
    ...
    private Map<String, Serializable> convertRawMap(Map rawMap, String artifact) {
        ...
        @SuppressWarnings(value = { "unchecked" }) Set<Map.Entry> entrySet = rawMap.entrySet();
        for (Map.Entry entry : entrySet) {
            Object rawKey = entry.getKey();
            if (!(rawKey instanceof String)) {
                throw new RuntimeException("Artifact {" + artifact + "} has invalid tag {" + rawKey + "}:" + " Value cannot be cast to String");
            }
            Object rawValue = entry.getValue();
            if (!(rawValue instanceof Serializable)) {
                throw new RuntimeException("Artifact {" + artifact + "} has invalid value {" + rawValue + "}:" + " Value cannot be cast to Serializable");
            }
