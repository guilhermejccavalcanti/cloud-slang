...
public class PreCompilerErrorsTest {
    ...
    public void testOperationWithNullFileName() throws Exception {
        ...
        assertTrue(result.getErrors().size() > 0);
        exception.expect(RuntimeException.class);
        exception.expectMessage("should be declared in a file named \"test_op\" " + "plus a valid extension(sl, sl.yaml, sl.yml, prop.sl, yaml, yml)");
        throw result.getErrors().get(0);
    }
    ...
    public void testOperationWithWrongNameWrongExtension() throws Exception {
        ...
        assertTrue(result.getErrors().size() > 0);
        exception.expect(RuntimeException.class);
        exception.expectMessage("it should be declared in a file named \"test_op\" " + "plus a valid extension(sl, sl.yaml, sl.yml, prop.sl, yaml, yml)");
        throw result.getErrors().get(0);
    }
    ...
    public void testFlowSameInputAndOutputName() throws Exception {
        ...
        assertTrue(result.getErrors().size() > 0);
        exception.expect(RuntimeException.class);
        exception.expectMessage("Inputs and outputs names should be different for \"io.cloudslang.base.json.same_input_and_output_name\". " + "Please rename input/output \"json_path\"");
        throw result.getErrors().get(0);
    }
    ...
    public void testWrongTag() throws Exception {
        ...
        assertTrue(result.getErrors().size() > 0);
        exception.expect(RuntimeException.class);
        exception.expectMessage("Artifact {private_input_without_default} has unrecognized tag {action}. " + "Please take a look at the supported features per versions link");
        throw result.getErrors().get(0);
    }
    ...
    public void testFlowWithResultExpressions() throws Exception {
        ...
        validateExceptionMessage(errors.get(0), "Flow: \'flow_with_result_expressions\' syntax is illegal. Error compiling result: \'SUCCESS\'. " + "Explicit values are not allowed for flow results. Correct format is: \'- SUCCESS\'.", "SUCCESS", PreCompileValidatorImpl.FLOW_RESULTS_WITH_EXPRESSIONS_MESSAGE);
        validateExceptionMessage(errors.get(1), "Flow: \'flow_with_result_expressions\' syntax is illegal. Error compiling result: \'CUSTOM\'. " + "Explicit values are not allowed for flow results. Correct format is: \'- CUSTOM\'.", "CUSTOM", PreCompileValidatorImpl.FLOW_RESULTS_WITH_EXPRESSIONS_MESSAGE);
    }

    private void validateExceptionMessage(RuntimeException ex, String flowName, String resultName, String expressionMessage) {
    ...
    public void testFlowOnFailureSkipped() throws Exception {
        ...
        assertTrue(result.getErrors().size() > 0);
        exception.expect(RuntimeException.class);
        exception.expectMessage("Error compiling source \'flow_on_failure_skipped\'.\n" + "Flow: \'flow_on_failure_skipped\' has steps with keyword on the same indentation as the step name or " + "there is no space between step name and hyphen.");
        throw result.getErrors().get(0);
    }
    ...
    public void testFlowBothResultAndStepName() throws Exception {
        ...
        assertTrue(result.getErrors().size() > 0);
        exception.expect(RuntimeException.class);
        exception.expectMessage("Navigation target: \'BOTH_RESULT_AND_STEP_NAME\' is declared both as step name and flow result.");
        throw result.getErrors().get(0);
    }
    ...
    public void testFlowWithInvalidInputs() throws Exception {
        ...
        assertTrue(result.getErrors().size() > 0);
        exception.expect(RuntimeException.class);
        exception.expectMessage("For flow \'flow_with_invalid_inputs\' syntax is illegal.\n" + "Invalid syntax after input \"input1\". Please check all inputs are provided as a list " + "and each input is preceded by a hyphen. Input \"input2\" is missing the hyphen.");
        throw result.getErrors().get(0);
    }
    ...
    private void assertContains(List<RuntimeException> errors, int index, String message) {
        @SuppressWarnings(value = { "all" }) RuntimeException rex1 = errors.get(index);
